...
<input type="hidden" id="fecha-app2" value="<?= date("Y-m-d", strtotime(date("Y-m-d") . ' + 1 year')); ?>">
...
<div class="row" id="container-vue">
								...
								<tbody>
									<tr v-for="(item,index) in productos">
										...
										<td><span v-if="!item.edicion">{{item.serie_producto}}</span><input
												v-if="item.edicion" @click="MultipleSerie(item.codigo,item.cantidad,index)"
												v-model="item.serie">
										</td>
										<td>
											...
											<div class="row">
												...
												<div class="col-md-6">
													<div class="form-group ">
														...
														<div class="col-lg-12">
															<input class="form-control text-center" v-model="venta.tc" type="text">
														</div>
													</div>
												</div>
											</div>
											...
											<div class="row">
												...
												<div class="col-md-6">
													<div class="form-group ">
														...
														<div class="col-lg-12">
															<input v-model="venta.fechaVen" type="date"
																placeholder="dd/mm/aaaa" name="input_fecha"
																class="form-control text-center" value="2021-10-16">
														</div>
													</div>
	...
	<div class="modal fade" id="modalSelMulti" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
		<div class="modal-dialog  modal-lg">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="exampleModalLabel">Seleccione Serie</h5>
				...
				<div class="modal-body">
					<input v-model="dataKeyndex" id="dataKeyndex" type="hidden" class="form-control">
					<input v-model="dataKeyCANTs" id="dataKeyCANTs" type="hidden" class="form-control">

					<div class="mb-3">
						<input v-model="dataKeySeries2" id="dataKeySeries2" name="dataKeySeries2" type="text" class="form-control">
					</div>

					<table id="datatableProductoDetalle2" class="table table-bordered dt-responsive  text-center table-sm" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
						<thead>
							<tr>
								<th style="text-align: center;">#</th>
								<th style="text-align: center;">Serie</th>
				...
				<div class="modal-footer">
					<button @click="GuardarSerie(dataKeyndex,dataKeySeries2)" class="btn btn-success">Agregar</button>
					<button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
				</div>
			</div>
		</div>
...
<script>
	...
	$(document).ready(function() {
		...
		const app = new Vue({
			...
			methods: {
				GuardarSerie(index, addSerie) {
					const inputValue = document.getElementById('dataKeySeries2').value;

					Vue.set(this.productos, index, {
						...this.productos[index],
						serie_producto: inputValue,
						serie: inputValue

					});

					$("#modalSelMulti").modal('hide');
					this.dataKeyCANTs = 0;
					this.dataKeySeries2 = '';
					this.selected2 = [];
				...
				busquedaKeyPess(evt) {
					...
					if (this.dataKey.length > 0) {
						_get("/ajs/cargar/productosv/" + app.almacenBusqueda + "?term=" + this.dataKey, (result) => {
							console.log(result)
				...
				MultipleSerie(index, cnt, filtro) {
					const cod = index;
					this.dataKeyCANTs = cnt;
					this.dataKeyndex = filtro;
					console.log(filtro);
					...
					_ajax("/ajs/data/producto/infos", "POST", {
						...
						function(resp) {
							if (resp.res) {
								...
								datatableProductoDetalle = $("#datatableProductoDetalle2").DataTable({
									paging: true,
									bFilter: true,
									ordering: true,
									searching: true,
									...
									columns: [{
										...
										{
											"defaultContent": `
								<button class='btn btn-success btn-sm mdi mdi-plus  btnSeless2' data-toggle='tooltip' data-placement='top' title='Seleccionar'></button> 
						      `
										}
									],
								});
							...
							_post("/ajs/data/producto/info/code", {
								...
								function(resp) {
									...
									if (resp.res) {
										...
										ui.item.precio_unidad = ui.item.precio_unidad;
		...
		$("#datatableProductoDetalle2").on("click", ".btnSeless2", function(e) {
			e.preventDefault();
			let cventa = $('#dataKeyCANTs').val();
			fila = $(this).closest("tr");
			Newserie = fila.find('td:eq(1)').text();
			...
			if (app.selected2.length < cventa) {
				...
				listarSerie = app.selected2.join(",");
				console.log(listarSerie);
				$('#dataKeySeries2').val(listarSerie);
			} else {
		...
		$("#input_buscar_productos").autocomplete({
			...
			source: function(request, response) {
				let url = _URL + "/ajs/cargar/productosv/" + app.almacenBusqueda;
				$.getJSON(url, {
					term: request.term
				}, response);
			},
